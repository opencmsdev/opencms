...
Every interactive element is a pill (`{rounded.pill}` 9999 px) with 1 px white-translucent border `rgba(255, 2555, 0.25)`. The button shape never varies — the same translucent-white pill carries "Try Grok", "Read announcement", "Custom Voices", "Sign up now", and every "Read" anchor. The pill is the entire shape system.
...
- Cards are tight `{rounded` 8 px rectangles in a slightly-lighter `{colors.canvas-card}` (`#191919`) fill with hairline border. No shadows.
...
- **Twilight.accent-twilight}` — `#c4b5fd`): Soft violet — illustrative accent.
...
- **Ink Hover** (`{colors.ink-hover}` — `#fafaf7`): Slightly off-whiteltered out per no-hover policy in component specs).
...
1. **universalSans** — proprietary geometric sans used for every display, body, button, and link role. Weight 400 only on the marketing surface (the brand's restraint is part of the voice). Neter-spacing at display sizes is the visual signature.
...
| `{typography.display-sm}` | 32px | 400 | 36px | -0 Card-cluster headings. |
...
- **Mono** — *Geist Mono* is the documentednd companion; *JetBrains Mono* or *IBM Plex Mono* are alternates.
...
#### Image Behavior
The brand uses sparse SVG illustract moments (Grok, Voice, API). No photography on the marketing surface.
...
- Background `{colors.primary}` white, text `{colors.on-primary}` near-black, 1 px solid white border, label `{typography.button-md}`, padding `{spacing.xs} {spacing.md}`, shape `{rill}` 9999 px.
...
- Background `{colors.canvas-soft}`, texnk}`, 1 px solid `{colors.hairline}`, body in `{typography.body-md}`, padding `{spacing.md} {spacing.lg}`, shape `{rounded.sm}` 8 px.
...
- Background `{colors.canvas}`, text `{colors.ink}`, padding `{spacing.4xl} {spacing.xl}`. Section headline in `{typography.display-md}` preceded by a `{typography.capt UPPERCASE GeistMono eyebrow.
...
**`ex-pricing-tier-featured`** — Featured/highlighted tier — polarity-flipped surface (dark fill + light text in light mode, light fill + dark texte).
...
- Properties: `backgroundColor`, `ding`
...
### Don't
-duce a light-mode counterpart. xAI is dark-canvas only.
